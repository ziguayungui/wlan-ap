...
static void nl80211_scan_del(struct nl80211_scan *nl80211_scan)
{
	LOGD("%s: delete scan context. avl:%p. scan_ctx:%p",
		nl80211_scan->name, nl80211_scan, nl80211_scan->scan_ctx);
	ev_async_stop(EV_DEFAULT, &nl80211_scan->async);
	avl_delete(&nl80211_scan_tree, &nl80211_scan->avl);
	nl80211_scan->scan_ctx = NULL;
	free(nl80211_scan);
}
...
static void nl80211_scan_finish(char *name, bool state)
{
	...
	if (nl80211_scan) {
		...
		avl_for_each_element(&nl80211_scan_tree, avl_entry, avl) {
			LOGD("nl80211_scan_finish. AVL list: %p\n", avl_entry);
		}
